...
export default function RedFlagBanner({ flags }: { flags: RedFlag[] }) {
  ...
  return (
    <div
      role="alert"
      className="rounded-sm border border-weak bg-weak-bg px-4 py-4 sm:px-5"
    >
      <p className="font-mono text-xs uppercase tracking-[0.1em] text-weak">
        {flags.length} potential red flag{flags.length === 1 ? "" : "s"} in
        this listing
      </p>
      ...
      <p className="mt-3 border-t border-weak/30 pt-3 font-mono text-[11px] text-ink-faint">
